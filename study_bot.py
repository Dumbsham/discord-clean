...
def ensure_user(conn, user_id: int, guild_id: int):
    conn.execute(
...
def reset_periods(conn, user_id: int, guild_id: int):
    """Reset daily/weekly XP counters when a new day/week starts."""
    row = conn.execute(
...
def add_xp(conn, user_id: int, guild_id: int, minutes: float):
...
def active_session(conn, user_id: int, guild_id: int):
    return conn.execute(
...
# ── In-memory active sessions {(user_id, guild_id): start_timestamp} ──────────
# Also stored in DB for persistence across restarts.
# ─────────────────────────────────────────────────────────────────────────────
...
@study_group.command(name="start", description="Start a study session")
async def study_start(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        f"📚 Study session started! Use `/study stop` when you're done.",
        ephemeral=False
    )
...
async def study_status(interaction: discord.Interaction):
    ...
    with get_db() as conn:
        ...
        xp_preview = int(elapsed * XP_PER_MINUTE)
...
async def leaderboard(interaction: discord.Interaction, period: str = "daily"):
    ...
    with get_db() as conn:
        # Reset periods for all active users first
        rows = conn.execute(
    ...
    embed.set_footer(text=f"Top {len(lines)} studiers • XP = {XP_PER_MINUTE}/min")
    await interaction.response.send_message(embed=embed)
